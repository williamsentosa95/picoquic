...
def run_emulated_browser(mp_option):
    cmd = []
    mm_multipath_cmd = get_mm_multipath_cmd()
    
    emulated_web_browsing_mp_program = BASE_FOLDER + "/emulated_http3_browser_mp"
    emulated_web_browsing_sp_program = BASE_FOLDER + "/emulated_http3_browser"
    sitename = "0_www.glassdoor.com.json"
    net_logfpath = HOME_FOLDER + "/picoquic-log/glassdoor-net.csv"
    emulated_web_browsing_cmd = []
    
    if (mp_option >= 0):
        emulated_web_browsing_cmd = [emulated_web_browsing_mp_program, WEB_BROWSING_BASE_FOLDER + sitename, str(mp_option), net_logfpath]
    else:
        emulated_web_browsing_cmd = [emulated_web_browsing_sp_program, WEB_BROWSING_BASE_FOLDER + sitename]

    cmd = mm_multipath_cmd + emulated_web_browsing_cmd
...
def main(args):
    ...
    client_process = run_emulated_browser(mp_option)
