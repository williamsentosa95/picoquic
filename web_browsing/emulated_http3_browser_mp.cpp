                ...
                if (fin_or_event == picoquic_callback_stream_fin) {
                    if (client_close_stream(cnx, ctx, stream_ctx)) {
                        ...
                        if (it != ongoing_requests.end()) {
                            ...
                            mimeType = ongoing_requests[key].mimeType;
                        }
                        ...
                        if (fp != NULL) {
                            fprintf(fp, "%d,%s,%d,%d,%s,%d,%f,%f\n", ctx->cnx_id, activity_id.c_str(), stream_id, stream_ctx->received_length, mimeType.c_str(), priority, network_time, queue_time);
                        }
...
int add_request_to_client(int filesize, int priority, string mimeType, string url, int cnx_id, string activity_id, int thread_id) {
    int ret = 0;
    bool found = false;
    ...
    for (int i=0; i<quic_cnxs.size(); i++) {
        if (quic_cnxs[i]->cnx_id == cnx_id) {
            found = true;
            quic_cnxs[i]->request_queue->push({filesize, priority, mimeType, url, cnx_id, activity_id, thread_id, chrono::steady_clock::now()});
        }
    } 
    
    if (!found) {
        quic_connection * quic_cnx = create_and_start_quic_connections(qclient, &server_addr, &config, quic_cnxs.size());
        quic_cnx->request_queue->push({filesize, priority, mimeType, url, cnx_id, activity_id, thread_id, chrono::steady_clock::now()});
...
void browser(int thread_id, set<string> cp) {
    int count = 0;
    Job job;
    bool acquired = false;
        ...
        if (acquired) {
            ...
            if (is_sane(job.id, activities[job.id])) {
                if (job.id.find("Networking") != string::npos) {
                    ...
                    add_request_to_client(size_bytes, priority, mimeType, path, conn_id, job.id, thread_id);
...
int main(int argc, char *argv[]) {
    ...
    int picoquic_mp_scheduling_mode = 0;
    if (argc > 2) {
        picoquic_mp_scheduling_mode = stoi(argv[2]);
    }

    if (argc > 3) {
        fp = fopen( argv[3], "w");
        if (fp != NULL) {
            fprintf(fp, "%s,%s,%s,%s,%s,%s,%s\n", "cnx_id", "activity_id", "stream_id", "data_size", "mimeType", "priority", "transfer_time", "queue_time");
        }
    }

    if (argc > 4) {
        client_msg_log_fp = fopen( argv[4], "w" );
        if (client_msg_log_fp != NULL) {
            fprintf(client_msg_log_fp, "%s,%s,%s,%s\n", "arrival_time_s", "msg_size_byte", "cnx_id", "msg_id");
        }
    }

    if (picoquic_mp_scheduling_mode) {
        picoquic_enable_mp_scheduling(picoquic_mp_scheduling_mode);
    ...
    set<string> cp = {"Networking_0", "Networking_50"};
    ...
    for (int i = 0; i < NUM_THREADS; ++i) {
        threadPool.emplace_back(browser, i, cp);
    }
